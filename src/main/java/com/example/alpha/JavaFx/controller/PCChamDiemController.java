    ...
    public void initialize(URL url, ResourceBundle resourceBundle){
        ...
        Model.getInstant().getViewFactory().getNamHoc().addListener((observable, oldValue, newValue) -> {
            ...
            for (int i=1; i<phanCongList.size(); i++) {
                ...
                if(!Model.getInstant().getCellGiaoVien().getMaGV().get().contains(phanCongList.get(i).getMaGiaoVien()) && Objects.equals(phanCongList.get(i).getMaNamHoc(), newValue)) {
...
//                VBox_GV.getChildren().clear();
//                VBox_GV.getChildren().addAll(MaGV.values());
            }
        });
    }
}